...
ap.add_argument("-m", "--ssd-ppn", default = '../models/ssd-ppn',
	help="base path to ssd-ppn directory")

args = vars(ap.parse_args())

labelsPath = os.path.sep.join([args["ssd_ppn"],
	"object_detection_classes_coco.txt"])
LABELS = open(labelsPath).read().strip().split("\n")

print("[INFO] loading ssd-ppn from disk...")
# Load a model imported from Tensorflow
tensorflowNet = cv2.dnn.readNetFromTensorflow('../models/ssd-ppn/frozen_inference_graph.pb', '../models/ssd-ppn/graph.pbtxt')
...
print("[INFO] SSD-PPN took {:.6f} seconds".format(end - start)) 
